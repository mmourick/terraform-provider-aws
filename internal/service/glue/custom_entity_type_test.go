...
func TestAccCustomEntityType_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy:             testAccCheckCustomEntityTypeDestroy(ctx),
...
func TestAccCheckCustomEntityTypeExists(ctx context.Context, resourceName string, customEntityType *glue.GetCustomEntityTypeOutput) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if !ok {
			return create.Error(names.Glue, create.ErrActionCheckingExistence, tfglue.ResNameCustomEntityType, resourceName, errors.New("not found"))
		}

		if rs.Primary.ID == "" {
			return create.Error(names.Glue, create.ErrActionCheckingExistence, tfglue.ResNameCustomEntityType, resourceName, errors.New("not set"))
		}

		conn := acctest.Provider.Meta().(*conns.AWSClient).GlueClient(ctx)

		resp, err := tfglue.FindCustomEntityType(ctx, conn, rs.Primary.Attributes[names.AttrName])
		if err != nil {
			return create.Error(names.Glue, create.ErrActionCheckingExistence, tfglue.ResNameCustomEntityType, rs.Primary.ID, err)
		}

		*customEntityType = *resp

		return nil
	}
}

func testAccCheckCustomEntityTypeDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
